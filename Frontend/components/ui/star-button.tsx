...
export function StarButton({
  ...
  useEffect(() => {
    ...
    if (!button || !cone) return;

    // Size the rotating cone to be large enough to reach all 4 corners
    const w = button.offsetWidth;
    const h = button.offsetHeight;
    const size = Math.sqrt(w * w + h * h) * 1.2;

    cone.style.width = `${size}px`;
    cone.style.height = `${size}px`;
    cone.style.top = `${h / 2 - size / 2}px`;
    cone.style.left = `${w / 2 - size / 2}px`;

    // Spin the cone — overflow:hidden on button clips it to show only the border edge
    const anim = cone.animate(
  ...
  return (
    <button
      ...
      {...props}
    >
      {/* Spinning conic glow — clipped by overflow:hidden to appear as border beam */}
      <div
        ref={coneRef}
        className="absolute pointer-events-none rounded-full"
        style={{
          background: `conic-gradient(from 0deg, transparent 75%, ${lightColor} 88%, ${lightColor} 92%, transparent 100%)`,
          opacity: 0.9,
        } as CSSProperties}
      />
      ...
      {/* Label */}
      <span className="z-10 relative text-white font-semibold tracking-wide">
        {children}
      </span>
    </button>
  );
